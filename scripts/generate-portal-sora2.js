...
/**
 * Portal intro loop via OpenAI Sora 2 → assets/video/portal-loop.mp4
...
async function createVideoJob(apiKey, brief) {
  ...
  form.append('seconds', String(brief.seconds || '8'));
...
async function main() {
  ...
  console.log('Sora 2 Portal-Prompt:\n', brief.prompt, '\n');
  const outPath = path.join(ROOT, brief.output || 'assets/video/portal-loop.mp4');
  console.log('creating Sora 2 portal job…');
